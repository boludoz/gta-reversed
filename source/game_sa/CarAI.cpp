...
void CCarAI::UpdateCarAI(CVehicle* veh) {
    ...
    case STATUS_GHOST: {
            ...
            if (   !plyrVeh // 0x41DE95 | 0x41E1F0
                ...
                || plyrVeh->GetMoveSpeed().SquaredMagnitude() < maxSpeedSq && veh->m_nCopsInCarTimer > 2500
                        ...
                        if (veh->vehicleFlags.bIsLawEnforcer) { // 0x41DED4 | 0x41E23F
                            if ((veh->GetModelIndex() != MODEL_RHINO || veh->m_nRandomSeed > 10'000) && vehPlyrDist2DSq <= sq(10.f)) { // 0x41DEE1 | 0x41E254
        ...
        case MISSION_APPROACHPLAYER_CLOSE: { // 0x41F12D
            ...
            if (vehToPlyrDist2DSq <= sq(FindSwitchDistanceFar(veh))) {
                ...
                if ((!plyrVeh || plyrVeh->IsUpsideDown() || veh->m_nCopsInCarTimer >= (veh->GetModelIndex() == MODEL_COPBIKE ? 2'500 : 20'000)) && veh->vehicleFlags.bIsLawEnforcer && vehToPlyrDist2DSq <= sq(10.f)) {
                    TellOccupantsToLeaveCar(veh);
                    ap->ClearCarMission();
                    ap->SetCruiseSpeed(0);
                ...
                } else if (veh->GetModelIndex() == MODEL_COPBIKE && veh->m_pDriver) {
    ...
    if (veh->vehicleFlags.bIsLawEnforcer) {
        if (FindPlayerWanted()->GetWantedLevel() >= 1) {
            if (CCullZones::CurrentFlags_Player & eZoneAttributes::ZONE_ATTRIBUTE_CAMCLOSEIN_FORPLAYER) {
                TellOccupantsToLeaveCar(veh);
                ap->SetCarMission(MISSION_NONE);
                ap->SetCruiseSpeed(0);
            }
